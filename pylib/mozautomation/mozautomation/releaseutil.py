...
def import_nightly_builds(database_path, repo, start_day=None):
    ...
    if not start_day:
        if b"last_nightly_day" in state:
            ...
            print(
                "using %s as start day calculated from last seen day %s"
                % (start_day, last_day)
            )
        else:
            # This is the earliest day we can import Nightly releases for.
            start_day = datetime.date(2010, 4, 1)
            print("using %s as Nightly start day because no state seen" % start_day)

    nightly_builds = releasescraper.find_nightly_builds(start_day)
    nightly_builds = releasescraper.ensure_full_revision(nightly_builds, repo)
