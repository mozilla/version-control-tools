...
class TestLandoRequiredCheck(TestCase):
    ...
    a_set_of_scm_level_3_group_affiliations = {
        "group_1",
        lando_required.ACTIVE_SCM_LEVEL_3,
        "group_3",
        "group_4",
    }
    a_set_of_scm_allow_direct_push_group_affiliations = {
        ...
        "group_2",
        "group_3",
        lando_required.ACTIVE_SCM_ALLOW_DIRECT_PUSH,
    }
    ...
    def test_magicwords_and_justification_regular_expression(self):
        sample_source_and_expected_result = (
            ...
            ("%s because I want to" % lando_required.MAGIC_WORDS, True),
            (
            ...
            (
                ...
                "%s because I want to" % lando_required.MAGIC_WORDS,
                True,
            ),
            (
                "standard boring changeset (Bug 12345) r=lars\n"
                ...
                "%s\n"
                "because I want to" % lando_required.MAGIC_WORDS,
                True,
            ),
            (
                "standard boring changeset (Bug 12345) r=lars\n"
                "and this is multiline - with missing justification\n"
                "%s" % lando_required.MAGIC_WORDS,
                False,
            ),
        )
        ...
        for source, expected_result in sample_source_and_expected_result:
            self.assertEqual(
                bool(
                    lando_required.MAGICWORDS_WITH_JUSTIFICATION_RE.search(source)
                ),
                expected_result,
    ...
    def test_relevant__success(self):
        ...
        check_object = lando_required.LandoRequiredCheck(ui=ui_mock, repo=repo_mock, info=Mock())
        # the tested call
        result = check_object.relevant()
        # verifying test results
        ui_mock.config.assert_called_with("mozilla", "lando_required_repo_list")
    ...
    @mock.patch("mozhghooks.check.lando_required.sentry_sdk.init")
    @mock.patch("mozhghooks.check.lando_required.sentry_sdk.capture_message")
    def test_log_push_attempt__success_using_sentry(
        ...
        check_object = lando_required.LandoRequiredCheck(ui=ui_mock, repo=Mock(), info=Mock())
        # the tested call
    ...
    @mock.patch("mozhghooks.check.lando_required.sentry_sdk.init")
    @mock.patch("mozhghooks.check.lando_required.sentry_sdk.capture_message")
    @mock.patch("mozhghooks.check.lando_required.print_banner")
    def test_log_push_attempt_sentry_failure(
        ...
        print_banner_mock.assert_called_with(
            ui_mock,
            "warning",
            lando_required.SENTRY_FAILURE_WARNING_MESSAGE % repr(raised_exception),
        )

    @mock.patch("os.environ.get")
    @mock.patch("mozhghooks.check.lando_required.get_active_scm_groups")
    def test_get_user_and_group_affiliations__for_valid_username(
        ...
        # the tested method
        result = lando_required.get_user_and_group_affiliations()
        # verifying test results
    ...
    def testchangeset_has_justification__for_no_magic(self):
        """test userchangeset_has_justification_and_can_push for the case where SCM Level 3 is present
        ...
        # the tested call:
        result = lando_required.changeset_has_justification(description)
        # verifying test results
        self.assertEqual(result, False)

    def testchangeset_has_justification__for_magic_justification(self):
        """test userchangeset_has_justification_and_can_push for the case where SCM Level 3 is present
        ...
        ) % lando_required.MAGIC_WORDS
        # the tested call:
        result = lando_required.changeset_has_justification(description)
        # verifying test results
        self.assertEqual(result, True)

    def testchangeset_has_justification__for_magic_no_justification(self):
        """test userchangeset_has_justification_and_can_push for the case where SCM Level 3 is present
    ...
    @mock.patch(
        "mozhghooks.check.lando_required.get_user_and_group_affiliations"
    )
    @mock.patch("mozhghooks.check.lando_required.print_banner")
    def test_pre__for_ldap_exception(
        ...
        print_banner_mock.assert_called_with(
            ui_mock,
            "error",
            lando_required.LDAP_USER_EXCEPTION_FAILURE_MESSAGE
    ...
    def test_pre__for_ldap_no_user(
        ...
        # verifying test results
        print_banner_mock.assert_called_with(
            ui_mock, "error", lando_required.LDAP_USER_FAILURE_MESSAGE
        )
    ...
    def test_pre__for_no_privileged_group(
        ...
        print_banner_mock.assert_called_with(
            ui_mock, "error", lando_required.INSUFFICIENT_PRIVILEGE_FAILURE_MESSAGE
        )
    ...
    def test_pre__for_scm_level_1_group(
        ...
        check_object = lando_required.LandoRequiredCheck(ui=Mock(), repo=Mock(), info=Mock())
        # the tested call:
        check_object.pre("node")
        # verifying test results
        self.assertEqual(check_object.privilege_level, lando_required.ACTIVE_SCM_LEVEL_3)
    ...
    def test_pre__for_scm_allow_direct_push_group(
        ...
        # verifying test results
        self.assertEqual(
            check_object.privilege_level, lando_required.ACTIVE_SCM_ALLOW_DIRECT_PUSH
        )
        print_banner_mock.assert_not_called()

    @mock.patch(
        "mozhghooks.check.lando_required.changeset_has_justification"
    )
    @mock.patch("mozhghooks.check.lando_required.print_banner")
    ...
    def test_check__for_unknown_privilege(
        ...
        has_justification_mock.assert_not_called()
        print_banner_mock.assert_called_with(
            ui_mock, "error", lando_required.INTERNAL_ERROR_MESSAGE
        )
    ...
    def test_check__for_scm_level_3__single_commit_without_justification(
        ...
        ui_mock = Mock()
        check_object = lando_required.LandoRequiredCheck(ui=ui_mock, repo=Mock(), info=Mock())
        check_object.privilege_level = lando_required.ACTIVE_SCM_LEVEL_3
        check_object.first_ctx_rev = None
        ctx_mock = MagicMock()
        ctx_mock.hex.return_value.__getitem__.return_value = "012345689AB"
        ctx_mock.children.return_value = []
        ...
        self.assertEqual(check_object.first_ctx_rev, "012345689AB")
        print_banner_mock.assert_called_with(
            ui_mock, "error", lando_required.SCM_LEVEL_3_PUSH_ERROR_MESSAGE
        )
    ...
    def test_check__for_scm_level_3__multiple_commit_with_justification(
        ...
        ctx_mock_list[
            -1
        ].description.return_value = "MANUAL PUSH: yes, I get to do this"
        # the tested call:
        for a_ctx in ctx_mock_list:
            result = check_object.check(a_ctx)
            # verify interim results
    ...
    @mock.patch(
        "mozhghooks.check.lando_required.LandoRequiredCheck._log_push_attempt"
    )
    @mock.patch("mozhghooks.check.lando_required.print_banner")
    def test_post_check__for_scm_allow_direct_push(
        ...
        log_push_attempt_mock.assert_called_with(
            lando_required.SUCCESS_FOR_SCM_ALLOW_DIRECT_PUSH_LOG_MESSAGE
    ...
    def test_post_check__for_scm_level_3(
        ...
        self.assertTrue(result)
        log_push_attempt_mock.assert_called_with(
            lando_required.SUCCESS_FOR_SCM_LEVEL_3_LOG_MESSAGE
            % (check_object.first_ctx_rev, check_object.user_name)
        )
        print_banner_mock.assert_not_called()
