...
# stream bundles generated by older hg may have extra requirements we should ignore
IGNORED_REQUIREMENTS = {"dotencode", "fncache", "store"}
# known set of current requirements
KNOWN_REQUIREMENTS = {
    "generaldelta",
...
def validate_compressed_bundlespec(expected):
    def validate_spec(spec, path):
        if spec != expected:
            raise Exception(f"unexpected bundle spec for {path}: {spec} != {expected}")

    return validate_spec


def validate_stream_bundlespec(spec, path):
    if not spec.startswith("none-v2;stream=v2;requirements%3D"):
        raise Exception(f"unexpected bundle spec for {path}: {spec}")
    rest = spec.split("%3D", 1)[1]
    requirements = set(rest.split("%2C"))
    requirements -= IGNORED_REQUIREMENTS
    requirements -= KNOWN_REQUIREMENTS
    if requirements:
        raise Exception(f"unexpected requirements for {path}: {requirements}")


CLONEBUNDLES_ORDER = [
    ("zstd-max", validate_compressed_bundlespec("zstd-v2")),
    ("zstd", validate_compressed_bundlespec("zstd-v2")),
    ("gzip-v2", validate_compressed_bundlespec("gzip-v2")),
    (
        "stream-v2",
        validate_stream_bundlespec,
    ),
]
...
GCP_HOSTS = (
    ...
    ("moz-hg-bundles-gcp-us-east1", "us-east1"),
)
