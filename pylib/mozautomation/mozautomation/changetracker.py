...
class ChangeTracker(object):
    ...
    def _create_schema(self, existing):
        if existing < 2 and not self.created:
            raise Exception(
                "Incompatible local database detected. Delete "
        ...
        with self._db:
            ...
            self._db.execute(
                "CREATE TABLE IF NOT EXISTS changeset_pushes ("
                "changeset BLOB, "
                "head_changeset BLOB, "
                "push_id INTEGER, "
                "tree_id INTEGER, "
            ...
            self._db.execute(
                "CREATE INDEX IF NOT EXISTS i_changeset_pushes_push_id "
                "ON changeset_pushes (tree_id, push_id)"
            )

            self._db.execute(
                "CREATE TABLE IF NOT EXISTS bug_changesets ("
                "bug INTEGER, "
                "changeset BLOB, "
            ...
            self._db.execute(
                "CREATE INDEX IF NOT EXISTS i_bug_changesets_bug "
    ...
    def tree_id(self, tree, url=None):
        with self._db:
            field = self._db.execute(
                "SELECT id FROM trees WHERE name=? LIMIT 1", [tree]
            ).fetchone()

            if field:
                return field[0]

            self._db.execute("INSERT INTO trees (name, url) VALUES (?, ?)", [tree, url])
    ...
    def last_push_id(self, tree):
        ...
        last_push_id = self._db.execute(
            "SELECT push_id FROM pushes WHERE "
            "tree_id=? ORDER BY push_id DESC LIMIT 1",
            [tree_id],
        ).fetchone()
        ...
        with self._db:
            for push_id, push in repo.push_info(start_id=last_push_id + 1):
                if not push["changesets"]:
                    continue
                self._db.execute(
                    "INSERT INTO pushes (push_id, tree_id, time, "
                    "user) VALUES (?, ?, ?, ?)",
