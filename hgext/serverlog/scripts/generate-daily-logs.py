...
subprocess.check_call(
    "cat %s | %s/totals-by-day.py > /var/log/hg/totals-by-day.%s"
    % (dest_log, HERE, yesterday.isoformat()),
    shell=True,
)

subprocess.check_call(
    "cat %s | %s/repo-totals-by-day.py > /var/log/hg/repo-totals-by-day.%s"
    % (dest_log, HERE, yesterday.isoformat()),
    shell=True,
)

subprocess.check_call(
    "cat %s | %s/totals-by-hour.py > /var/log/hg/totals-by-hour.%s"
    % (dest_log, HERE, yesterday.isoformat()),
    shell=True,
)

subprocess.check_call(
    "cat %s | %s/repo-totals-by-hour.py > /var/log/hg/repo-totals-by-hour.%s"
    % (dest_log, HERE, yesterday.isoformat()),
    shell=True,
)
