...
SPECIFIER = rb"(?:r|a|sr|rs|ui-r)[=?]"
R_SPECIFIER = rb"\br[=?]"
R_SPECIFIER_RE = re.compile(R_SPECIFIER)
REQUAL_SPECIFIER_RE = re.compile(rb"r=")
RQUESTION_SPECIFIER_RE = re.compile(rb"r\?")
...
IRC_NICK = rb"[a-zA-Z0-9\-\_.]*[a-zA-Z0-9\-\_]+"  # this needs to match irc nicks
BMO_IRC_NICK_RE = re.compile(rb":(" + IRC_NICK + rb")")

REVIEWERS_RE = re.compile(
    rb"([\s\(\.\[;,])"
    + rb"("  # before 'r' delimiter
    + SPECIFIER
    + rb")"
    + rb"("  # flag
...
def add_hyperlinks(s, bugzilla_url=b"https://bugzilla.mozilla.org/show_bug.cgi?id="):
    ...
    if github_repo:
        repl = rb'<a href="https://github.com/%s/issues/\1">#\1</a>' % github_repo
        s = re.sub(rb"#(\d+)", repl, s)

    # Bugzilla linking.
    bugzilla_link = rb'<a href="%s\2">\1</a>' % bugzilla_url
