...
HERE = pathlib.Path(os.path.normpath(os.path.abspath(os.path.dirname(__file__))))
...
def upgrade_repo(path, hg, dry_run=False):
    ...
    args = [
        ...
        "extensions.mozhooks=!",
        "--config",
        "extensions.vcsreplicator=!",
        "--config",
        "hooks.pretxnclose.populate_caches=true",
        "--config",
        "progress.delay=1.0",
        "--config",
        "progress.refresh=10.0",
        "--config",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-n", "--dry-run", action="store_true", help="Do not perform upgrade"
    )
    parser.add_argument("-j", type=int, default=1, help="Number of workers")
    parser.add_argument(
        "--with-hg", type=str, required=True, help="Mercurial driver to use for upgrade"
    )
