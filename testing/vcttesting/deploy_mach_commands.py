...
class DeployCommands(object):
    ...
    @Command("hgmo", category="deploy", description="Deploy hg.mozilla.org")
    @CommandArgument(
        "--skip-hgssh", action="store_true", help="Skip hgssh deployment if present"
    )
    @CommandArgument(
        "--skip-hgweb", action="store_true", help="Skip hgweb deployment if present"
    )
    @CommandArgument(
    ...
    @CommandArgument("repo", help="Repo to strip (path under hg.mozilla.org/)")
    @CommandArgument("rev", help="Revset of revisions to strip")
    @CommandArgument("--verbosity", type=int, help="How verbose to be with output")
    ...
    @CommandArgument("repo", nargs="*", help="Repositories to re-clone")
    @CommandArgument(
        "--repo-file", help="File containing list of repositories to re-clone"
    )
    ...
    @CommandArgument(
        "--instance", help="Instance type to bootstrap (hgweb, mirror or none)"
    )
    @CommandArgument(
        "--hgweb-workers",
        type=int,
